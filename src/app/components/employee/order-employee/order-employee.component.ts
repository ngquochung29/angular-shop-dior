...
@Component({
  selector: 'app-order-employee',
  templateUrl: './order-employee.component.html',
  styleUrls: ['./order-employee.component.scss']
})
export class OrderEmployeeComponent implements OnInit {
...
shippingFee: number = 0;
  ...
  readonly SHIPPING_FEES: Record<'express' | 'normal'|'counter', number> = {
    counter:0,
  express: 70_000,
  normal: 35_000,
};
...
private getShippingFee(): number {
  const method = this.orderForm.get('shipping_method')?.value as 'express' | 'normal'|'counter';
...
private updateCartFromCartItems(): void {
  ...
  quantity: item.quantity,
      ...
      if (this.orderData.payment_method === 'vnpay') {
        ...
        this.orderService.placeOrderEmployee(this.orderData).subscribe({
