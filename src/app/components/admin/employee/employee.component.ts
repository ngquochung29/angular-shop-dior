...
route = inject(ActivatedRoute);
  ...
  ngOnInit(): void {
    this.currentPage = Number(this.localStorage?.getItem('currentEmployeeAdminPage')) || 0;
    this.getEmployee(this.keyword, this.currentPage, this.itemsPerPage);
  }
   insertEmployee(){
    this.router.navigate(['/admin/employees/insert']);
  }
  ...
  user: 'Khách hàng'
    ...
    this.getEmployee(this.keyword.trim(), this.currentPage, this.itemsPerPage);
  }
  getEmployee(keyword: string, page: number, limit: number) {
    this.userService.getEmployee({ keyword, page, limit }).subscribe({      
